...
void Move(int map[25][25], int user_status[9], int *x, int *y, int *temp, int *walk, int itemBox[3]);
void OpenDoor(int map[25][25], int *walk);

int Fight(int *sta, int *partner, int *partner1_sta, int *partner2_sta, char *partner_name1, char *partner_name2, int *itemBox, char *enemy_name, char *user_name);
void attack(char *size, int *status, int partNum, int **partnerSta1, int **partenrSta2, char *partnerName, char *partnerName2, int attack_1);
void attack_enemy(char *size, int *enemystatus, int enemyattack);
int MadePartner(int *partNum, int **partnerSta1, int **partnerSta2, char *partnerName, char *partnerName2, int *enemySta, char *enemyname);
...
int BossFight(int *sta, int *partner, int *partner1_sta, int *partner2_sta, char *partner_name1, char *partner_name2, char *user_name, int *itemBox);
...
int main(void)
{
    ...
    char *monster[21] = {"현승몬", "규비몬", "수정몬", "수빈몬", "수연몬", "다능몬", "세정몬", "광선몬", "웅철몬", "인희몬", "하경몬", "민형몬", "병훈몬", "주연몬", "용민몬", "홍균몬", "준호몬", "상호몬", "수철몬", "지우몬", "승범몬"};
    int user_status[9] = {1000, 5, 200, 270, 0, 1000, 5, 1, 5};
    int itemBox[3] = {0, 0, 0};
    char *partner_name2 = " ";
    char *partner_name1 = " ";
    int partner = 0, partner1Sta = 0, partner2Sta = 0;
    char *user_name = "devmon";
    int x = 12, y = 13;
    ...
    while (1)
    {
        ...
        if (temp == 2 && (rand() % 9) <= 0)
        {
            ...
            int Fc = Fight(user_status, &partner, &partner1Sta, &partner2Sta, partner_name1, partner_name2, itemBox, enemy_name, user_name);
            switch (Fc)
            {
            case 0:
            {
        ...
        {
            BossFight(user_status, &partner, &partner1Sta, &partner2Sta, partner_name1, partner_name2, user_name, itemBox);
        }
        OpenDoor(map, &walk);
        printf("----------------------------------상태창-----------------------------------\n");
        ...
        Move(map, user_status, &x, &y, &temp, &walk, itemBox);
        system("clear");
    }
...
int Fight(int *sta, int *partner, int *partner1_sta, int *partner2_sta, char *partner_name1, char *partner_name2, int *itemBox, char *enemy_name, char *user_name) // 전투 메인 함수
{
    // char *monster[22] = {"현승몬", "규비몬", "수정몬", "수빈몬", "수연몬", "다능몬", "세정몬", "광선몬", "웅철몬", "인희몬", "하경몬", "민형몬", "병훈몬", "주연몬", "용민몬", "홍균몬", "준호몬", "상호몬", "수철몬", "지우몬", "승범몬"};
    int turn = 1, atk, run_turn = 0;
    ...
    enemy_sta[0] = rand() % 201 + 200 + enemyleveluphp;
    enemy_sta[1] = rand() % 51 + 80 + enemyleveluphp;
    enemy_sta[2] = rand() % 71 + 80 + enemyleveluphp;
    enemy_sta[3] = enemy_sta[0];
    while (1)
    {
        int attack_1 = -(rand() % (sta[2] - 49)) + sta[2];
        int enemyattack = -(rand() % (enemy_sta[1] - 49)) + enemy_sta[1];
        system("clear");
        int back_count = 0;
        // rfp = fopen("poketmon_1", "rt");
        ...
        { // 공격했을경우
            ...
            {
            case 1:
                enemy_sta[0] = enemy_sta[0] - attack_1 - *partner1_sta;
                break;
            case 2:
                enemy_sta[0] = enemy_sta[0] - attack_1 - *partner1_sta - *partner2_sta;
                break;
            default:
                enemy_sta[0] -= attack_1;
                break;
            }
            attack(art, sta, *partner, &partner1_sta, &partner2_sta, partner_name1, partner_name2, attack_1);
        }
        else if (atk == 2)
        { // 특수공격 햇을경우
            if (sta[1] > 0)
        ...
        system("clear");
        sta[0] -= enemyattack;
        attack_enemy(art, enemy_sta, enemyattack); // 몬스터 공격
...
void attack(char *size, int *status, int partNum, int **partnerSta1, int **partnerSta2, char *partnerName, char *partnerName2, int attack_1) // 기본 공격 컷씬
{
    FILE *rfp_attak_1;
    FILE *rfp_attack_2;
    int q = 0;
    while (q < 3)
    {
        rfp_attak_1 = fopen("attack_1", "rt");
        rfp_attack_2 = fopen("attack_1.1", "rt");
...
void S_attack(char *size, int *status, int partNum, int **partnerSta1, int **partnerSta2, char *partnerName, char *partnerName2) // 특수공격 컷씬
{
    ...
    FILE *rfp_Sattack3;
    FILE *rfp_Sattack4;
    rfp_Sattack1 = fopen("Sattack_1", "rt");
    rfp_Sattack2 = fopen("Sattack_1.1", "rt");
    rfp_Sattack3 = fopen("Sattack_1.2", "rt");
    rfp_Sattack4 = fopen("Sattack_1.3", "rt");
...
int MadePartner(int *partNum, int **partnerSta1, int **partnerSta2, char *partnerName, char *partnerName2, int *enemySta, char *enemyname) // 동료 만들기
{
    char *monster[22] = {"현승몬", "규비몬", "수정몬", "수빈몬", "수연몬", "다능몬", "세정몬", "광선몬", "웅철몬", "인희몬", "하경몬", "민형몬", "병훈몬", "주연몬", "용민몬", "홍균몬", "준호몬", "상호몬", "수철몬", "지우몬", "승범몬"};
    int n;
    switch (*partNum)
...
int BossFight(int *sta, int *partner, int *partner1_sta, int *partner2_sta, char *partner_name1, char *partner_name2, char *user_name, int *itemBox) // 보스 전투 메인 함수
{
    int turn = 1, atk, run_turn = 0;
    int enemy_sta[4] = {50000, 2023, 3000, 50000};
    int attack_1 = rand() % sta[2];
    char art[2000];
    FILE *boss_rfp;
    ...
    while (1)
    {
        ...
        { // 보스 일반공격
            sta[0] -= bossdmg;
            attack_enemy(art, enemy_sta, bossdmg);
        }
        if (sta[0] <= 0)
        { // 결과
            break;
...
{
    FILE *mana_rfp;
    mana_rfp = fopen("MP_recovery1", "rt");
...
int QuizMon1(int score)
{
    ...
    char *quiz1[7] = {
        "우리나라의 8.15 광복은 몇년도일까요?\n1. 1930 2. 1940 3. 1943 4. 1945 ",
        "빗살무늬토기는 어느 시대의 유물일까요? \n1. 석기 시대 2. 신석기 시대 3. 철기 시대 4. 청동기 시대 ",
        "신조어 분좋카의 뜻은?\n1. 분위기 좋은 카플 2.분말 좋은 카레 3. 분위기 좋은 카페 4. 분위기 좋게 나오는 카메라 ",
        "00은 게편 00에 들어갈 단어는?\n1. 조개 2. 황새 3. 가재 4. 소라 ",
        "흑인이 울면? 1. 흑흑 2. 흑인은 울지 않는다. 3. 블랙 4. 흙흙 ",
        "투표를 영어로? \n1. Choice 2. Pick 3. Cast 4. Vote ",
        "주식에서 소액을 운용하는 일반 투자자를 뭐라고 부를까요? \n1.베짱이 2. 개미 3. 독수리 4. 바퀴벌레 ",
    };
